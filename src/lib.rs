...
fn test_1() -> Result<(), anyhow::Error> {
    ...
    if Bytes::from_hex(INPUT)?.to_base64() != EXPECTED {
...
fn test_2() -> Result<(), anyhow::Error> {
    ...
    let result = Bytes::from(utils::bytes_xor(
        Bytes::from_hex(INPUT)?.get_ref(),
        Bytes::from_hex(BUFFER)?.get_ref(),
    ))
    .to_hex();
    if result != EXPECTED {
        return Err(anyhow!("unexpected result"));
...
fn test_3() -> Result<(), anyhow::Error> {
    ...
    let result = single_byte_xor::try_break(Bytes::from_hex(INPUT)?.get_ref())
...
fn test_6() -> Result<(), anyhow::Error> {
    ...
    let data = Bytes::from_base64(FILE.replace("\n", "").as_ref());
